...
cards is a second job. Wants: *tell me what I don't know before the exam tells me.*

### P2 — "David" — the curious professional
34, software architect. Reads 2 books / month. Forgets 80%. Wants: *a map of what
actually stuck, not what I felt stuck.*

### P3 — "The Judge" — hackathon evaluator
Time-boxed. Has a coupon code. Will abandon if anything feels broken, slow, or confusing
...
- **Won't** A form with labels. This is a *moment*, not data entry.
...
- **Must** Empty state is a *destination*, not an error. Copy + single upload affordance.
...
- **Must** Each answer updates the *neighbor* concepts via dependencies (Bayesian-ish propagation)
...
- **Should** "I don't know" / "Skip" are first-class answers and carry information
  (lower confidence *and* lower mastery estimate)
...
### F-09 Premium / Subscriptions
- **Must** Two tiers: Free, Pro (€5.99/mo, €49.99/yr)
- **Must** Feature-flagged via DB table `FeatureFlag` + edge middleware `can(user, feature)`
- **Must** Hackathon path: coupon codes grant Pro until `proUntil` date — no Stripe
- **Must** Pricing page is real, calm, single-column — but "Redeem coupon" is the primary
  CTA during the hackathon window
- **Should** BillingProvider abstraction (`noop` now, `stripe` later) — see `architecture.md`
- **Won't** Connect Stripe during the hackathon. Architecture must make it a 1-day swap.

**Acceptance** A judge redeems `JUDGE100` and immediately sees Pro-only features unlock;
refreshing keeps them; expiry restores Free after `proUntil`.
...
| Observability | Structured logs + error tracking (Sentry-ish, provider TBD by phase 8) |

---

## 5. MVP Definition (hackathon scope)

**In:** F-01, F-02 (desktop), F-03, F-04, F-05, F-06, F-07, F-08, F-09 (coupons only),
F-10 (account + i18n), F-11, F-12.

**Out of MVP (deferred to roadmap Horizon 2+):** collaborator maps, mobile-native,
OCR, multi-doc aggregate maps, billed Stripe subscriptions, push notifications.
...
7. Redeem `JUDGE100` in <10s and unlock Pro flags (more concept depth / Q budget).
8. Toggle ES UI; all strings localize.
9. Install as PWA; relaunch offline shows Workspace list.
10. Read no console errors and no `any` types during `pnpm typecheck && pnpm lint && pnpm build`.
