...
export function MindHeader({ name, emoji, subtitle, actions, className }: MindHeaderProps) {
  return (
    <header
      className={cn(
        'flex flex-col gap-2 border-b border-[var(--color-border)] bg-[var(--color-bg)] px-6 py-6 md:flex-row md:items-center md:justify-between md:px-8',
        className,
      )}
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl font-semibold tracking-tight text-[var(--color-fg)]">{name}</h1>
          {subtitle ? (
